...
void ThermalDiffusion::ShowConfig() {
  ...
  if(status.status==Constant) {
    ...
    //                 << this->kappa << " ."<< std::endl;
    idfx::cout << "Thermal Diffusion: ENABLED with constant Peclet number="
                    << Pe << "."<< std::endl;
    idfx::cout << "Optically thin cooling: ENABLED with constant tcool="
                    << tcool << "."<< std::endl;
...
void ThermalDiffusion::AddDiffusiveFlux(int dir, const real t, const IdefixArray4D<real> &Flux) {
  ...
  EquationOfState eos = *(this->eos);
    ...
    IdefixArray1D<real> x = this->data->x[IDIR];
    IdefixArray1D<real> z = this->data->x[KDIR];
  #endif
  #if GEOMETRY == POLAR
  ...
  idefix_for("ThermalDiffusionFlux",kbeg, kend, jbeg, jend, ibeg, iend,
      KOKKOS_LAMBDA (int k, int j, int i) {
        // Compute gradT and initial gradT
        real gradT, gradT0;

        gradT  = Vc(PRS,k,j,i) / Vc(RHO,k,j,i)
               - Vc(PRS,k-koffset,j-joffset,i-ioffset) / Vc(RHO,k-koffset,j-joffset,i-ioffset);

        real Pinit      = PressureEqm(x(i), z(k), etahatLoc);
        real rhoinit    = DensityEqm(x(i), z(k), etahatLoc, NsqInfLoc, Nsq0Loc, NsqWidthLoc, gammaIdealLoc);

        real Pinit1     = PressureEqm(x(i-ioffset), z(k-koffset), etahatLoc);
        real rhoinit1   = DensityEqm(x(i-ioffset), z(k-koffset), etahatLoc, NsqInfLoc, Nsq0Loc, NsqWidthLoc, gammaIdealLoc);

        gradT0 = Pinit/rhoinit - Pinit1/rhoinit1;
	...
	real gamma = eos.GetGamma(Vc(PRS,k,j,i),Vc(RHO,k,j,i));
        if(haveThermalDiffusion == UserDefFunction) {
          ...
          // Compute kappaConstant from Pe, additional factors to get units correct, 
	  // and such that dT/dt ~ - (rho0/rho)*(H^2Omega/Pe)*laplacian(T'), which is closer to the Bousinesq energy equation
          kappa = R_MU*RHOG/(gamma-1.0)*(HGAS*HGAS*OMEGA/PeLoc);
        }

        // Add thermal diffusion to the flux
        Flux(ENG,k,j,i) += -kappa*(gradT - gradT0)/R_MU; //Division by R_MU to convert P/rho to temperature

        // Compute total diffusion coefficient
        //real gamma = eos.GetGamma(Vc(PRS,k,j,i),Vc(RHO,k,j,i)); (moved up)
