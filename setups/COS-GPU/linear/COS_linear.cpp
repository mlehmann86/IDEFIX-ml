...
const double kappaT = R_MU * RHOG * HGAS * HGAS * OMEGA / ((gamma - 1.0) * Pe);

//Buoyancy profile
const double NsqInfty = -0.05;
const double Nsq0     = -0.05;
const double D        = 0.5;

//Domain size
const double Lx = 4.0 * HGAS;

// Perturbation Parameters
const double pert_amp = 1e-4;     // W = P'/rho at inner boundary (normalized by CS^2)
const double kz       = M_PI / HGAS;    // Vertical wavenumber

// // Initial Conditions and Eigenvalue
// const complex<double> s_dimless(0.012023902659185348, 0.9779770826462233);                   // Example eigenvalue
// const complex<double> s = s_dimless*OMEGA;
// const complex<double> W0(pert_amp*CS*CS, 0.0);        // Initial W
// const complex<double> theta0(3.85310885768181e-05, 1.7482468867048373e-05);              // Initial theta
// const complex<double> theta_x0(0.0, 0.0);             // Initial theta_x
// const complex<double> vx0(0.0, 0.0);                  // Initial vx
...
int odes(double x, const double y[], double dydx[], void *params) {
    ...
    std::complex<double> vy = -(1.0 / s) * (OMEGA / 2.0 + duy_dx(x)) * vx;
    std::complex<double> Q = W / cs2(x) - theta;
    std::complex<double> dW_dx = 2.0 * OMEGA * vy + Q * gr(x) - W * dlnrho_dx(x) - s * vx;

    std::complex<double> dtheta_dx = theta_x;

    std::complex<double> chix = chi(x);
    std::complex<double> divv = -s * Q - vx * dlnrho_dx(x);
    std::complex<double> dtheta_x_dx = (s / chix - LapT_T(x) + kz * kz) * theta + (vx / chix - 2.0 * theta_x) * dlnT_dx(x) + (gamma - 1.0) / chix * divv;

    std::complex<double> ii(0.0,1.0);
    std::complex<double> vz = -ii * kz * W / s;
    std::complex<double> dvx_dx = -(s * Q + vx * dlnrho_dx(x) + ii * kz * vz);
...
int system_of_equations(const gsl_vector *v, void *params, gsl_vector *f) {
    ...
    double dx= 1e-4;
    solve_ode(-Lx / 2, Lx / 2, dx, y_init, s_guess);
...
std::tuple<std::complex<double>, std::complex<double>> find_root() {
    ...
    gsl_vector_set(x_init, 0, 0);  // Initial guess for real part of s
    gsl_vector_set(x_init, 1, OMEGA);   // Initial guess for imaginary part of s
...
void solve_on_coarse_grid(double x_min, double x_max, int Nx, double dx) {
  ...
  //Solve the IVP to get variables on the coarse grid, solve iteratively from one x to the next
    ...
    for (int i = 0; i < Nx; ++i) {
        ...
        complex<double> vy = -(1.0 / s_final) * (OMEGA / 2.0 + duy_dx(x)) * vx;
        complex<double> Q = W / cs2(x) - theta;
        complex<double> vz = -complex<double>(0, 1) * kz * W / s_final;
...
// Main function
int main() {
    double x_min = -Lx / 2, x_max = Lx / 2, dx = 1e-4*HGAS;
