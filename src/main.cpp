...
int main( int argc, char* argv[] ) {
...

  {
    ...
    #endif
    // Get output path from the ini file
    std::string outputPath = input.Get<std::string>("Output", "path", 0);

    // Make sure directory exists
    std::filesystem::create_directories(outputPath);

    // Build unique log file name based on MPI rank
    std::stringstream logFileName;
    logFileName << outputPath << "/idefix." << idfx::prank << ".log";

    // Redirect stdout
    FILE *logFile = freopen(logFileName.str().c_str(), "w", stdout);
    if(logFile == nullptr) {
      std::cerr << "❌ Failed to redirect stdout to " << logFileName.str() << std::endl;
    }

    // Redirect stderr
    FILE *errFile = freopen(logFileName.str().c_str(), "a", stderr);
    if(errFile == nullptr) {
      std::cerr << "❌ Failed to redirect stderr to " << logFileName.str() << std::endl;
    }
    Output output(input, data);   //updated 02.04.2025
