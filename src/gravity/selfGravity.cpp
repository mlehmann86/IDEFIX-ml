...
// Add these lines at the top of selfGravity.cpp
extern real sgTaperMinGlob; // New
extern real sgTaperMaxGlob; // New
...
void SelfGravity::InitSolver() {
  ...
  // Use the NEW, WIDER taper boundaries for self-gravity
  const real rmin_taper = data->mygrid->xbeg[IDIR];
  const real rmax_taper = data->mygrid->xend[IDIR];
  const real sg_wkzMin  = sgTaperMinGlob;
  const real sg_wkzMax  = sgTaperMaxGlob;
  IdefixArray1D<real> x1 = data->x[IDIR];

  idefix_for("InitDensityWithTaper", data->beg[KDIR], data->end[KDIR],
                                     data->beg[JDIR], data->end[JDIR],
                                     data->beg[IDIR], data->end[IDIR],
    KOKKOS_LAMBDA (int k, int j, int i) {
      real rho_val = Vc(RHO, k, j, i);
      real taper_factor = 1.0;
      real R = x1(i);

      // Taper density to zero inside the WIDER self-gravity tapering zone
      if (R < sg_wkzMin) {
        if (R <= rmin_taper) {
            taper_factor = 0.0;
        } else {
            taper_factor = pow(sin(M_PI_2 * (R - rmin_taper) / (sg_wkzMin - rmin_taper)), 2.0);
        }
      }
      if (R > sg_wkzMax) {
        if (R >= rmax_taper) {
            ...
            taper_factor = pow(sin(M_PI_2 * (rmax_taper - R) / (rmax_taper - sg_wkzMax)), 2.0);
        }
      }
      rho_val *= taper_factor;

      density(k+koffset, j+joffset, i+ioffset) = rho_val;
    });
...
// HACK/FIX: Force mean subtraction for any dimension that is periodic.
// Our problem is periodic in X2, so this MUST be run.
  //if(this->isPeriodic == true) {
