...
#ifdef UNSTRAT
                // For unstratified disks, potential is 2D-like (no z-dependence)
                // This matches the FARGO3D implementation.
                #if GEOMETRY == POLAR || GEOMETRY == CYLINDRICAL || GEOMETRY == SPHERICAL
                  r = x1(i);
                #else
                  r = ONE_F;
                #endif
...
#else
                // Standard 3D potential for stratified disks
                #if GEOMETRY == POLAR
