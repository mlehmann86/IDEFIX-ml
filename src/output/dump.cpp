...
Dump::Dump(Input &input, DataBlock *datain) {
  ...
  // ✅ First check for Output:path
  if(input.CheckEntry("Output", "path") > 0) {
    outputDirectory = input.Get<std::string>("Output", "path", 0);
  }
  // ✅ If not found, fall back to dmp_dir if defined
  else if(input.CheckEntry("Output","dmp_dir") >= 0) {
