...
void ThermalDiffusion::AddDiffusiveFlux(int dir, const real t, const IdefixArray4D<real> &Flux) {
  ...
  // Get the member array into a local variable that can be captured by the lambda
  IdefixArray1D<real> T0 = this->T0;
  ...
  idefix_for("ThermalDiffusionFlux",kbeg, kend, jbeg, jend, ibeg, iend,
      KOKKOS_LAMBDA (int k, int j, int i) {
        ...
        // Use the captured local variable 'T0'
        real T_pert_i = Vc(PRS,k,j,i) / Vc(RHO,k,j,i) - T0(i);
        real T_pert_im = Vc(PRS,k-koffset,j-joffset,i-ioffset) / Vc(RHO,k-koffset,j-joffset,i-ioffset)
                       - T0(i-ioffset);
        gradT = T_pert_i - T_pert_im;
