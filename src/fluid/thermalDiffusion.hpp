...
// Add initialization of diffusivityFunc to nullptr for good practice
template <typename Phys>
ThermalDiffusion::ThermalDiffusion(Input &input, Grid &grid, Fluid<Phys> *hydroin):
  ...
  if(status.status == Constant) {
    ...
    this->kappaArr = IdefixArray3D<real>("ThermalDiffusionKappaArray",data->np_tot[KDIR],
                                                                 data->np_tot[JDIR],
                                                                 data->np_tot[IDIR]);
    // STEP 2 CHANGE: Allocate the T0 array.
    this->T0 = IdefixArray1D<real>("T0_equilibrium", data->np_tot[IDIR]);
  } else {
