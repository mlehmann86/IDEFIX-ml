...
// Add these lines at the top of planetarySystem.cpp
extern real wkzMinGlob;
extern real wkzMaxGlob;
...
// Replace the entire function in planetarySystem.cpp with this version
void PlanetarySystem::AddPlanetsPotential(IdefixArray3D<real> &phiP, real t) {
  ...
  // Make wkz parameters available to the kernel for tapering
  const real rmin_wkz = this->data->mygrid->xbeg[IDIR];
  const real rmax_wkz = this->data->mygrid->xend[IDIR];
...
#ifdef UNSTRAT
    real distPlanet = sqrt(xp*xp+yp*yp);
#else
    ...
    idefix_for("PlanetPotential",
      ...
      0, this->data->np_tot[IDIR],
        KOKKOS_LAMBDA (int k, int j, int i) {
          ...
          real potential_from_planet = 0.0;

#ifdef UNSTRAT
        real dist = ((xc-xp)*(xc-xp)+ (yc-yp)*(yc-yp));
#else
        real dist = ((xc-xp)*(xc-xp)+ (yc-yp)*(yc-yp)+ (zc-zp)*(zc-zp));
#endif
        ...
        switch(myPlanetarySmoothing) {
            case PLUMMER:
              {
                potential_from_planet += -Mcentral*qp/sqrt(dist+smoothing*smoothing);
                break;
              }
            case POLYNOMIAL:
              {
                real rmrp = sqrt(dist);
                if (rmrp/smoothing < 1) {
                  potential_from_planet += -(Mcentral*qp/rmrp)*(pow(rmrp/smoothing,4.0) - 2.0*pow(rmrp/smoothing,3.0)+ 2.0*rmrp/smoothing);
                } else {
                  potential_from_planet += -(Mcentral*qp/rmrp);
                }
                break;
              }
            default: // do nothing
...
#ifdef UNSTRAT
          potential_from_planet += Mcentral*qp*(xc*xp+yc*yp)/(distPlanet*distPlanet*distPlanet);
#else
          potential_from_planet += Mcentral*qp*(xc*xp+yc*yp+zc*zp)/(distPlanet*distPlanet*distPlanet);
#endif
        }

        // ==========================================================
        // NEW: Conditional Tapering Logic
        // Enable with -DTAPER=ON compiler flag
        // ==========================================================
        #ifdef TAPER
          real taper_factor = 1.0;
          real R = x1(i); // In 2D polar, cylindrical radius R is just x1

          // Inner taper zone: ramp potential smoothly from 1 down to 0
          if (R < wkzMin) {
              if (R <= rmin_wkz) {
                  taper_factor = 0.0;
              } else {
                  // Use a cos^2 profile for a smooth ramp-down
                  taper_factor = pow(sin(M_PI_2 * (R - rmin_wkz) / (wkzMin - rmin_wkz)), 2.0);
              }
          }

          // Outer taper zone: ramp potential smoothly from 1 down to 0
          if (R > wkzMax) {
              if (R >= rmax_wkz) {
                  taper_factor = 0.0;
              } else {
                  taper_factor = pow(sin(M_PI_2 * (rmax_wkz - R) / (rmax_wkz - wkzMax)), 2.0);
              }
          }
          // Apply the taper to the calculated potential
          phiP(k,j,i) += potential_from_planet * taper_factor;
        #else
          // If TAPER is not defined, apply the potential normally
          phiP(k,j,i) += potential_from_planet;
        #endif // TAPER
    });
  }
