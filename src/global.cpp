...
#include "output.hpp"  // Needed because we use Output in enableLogFile()
...
void IdefixOutStream::enableLogFile(const Output& output) {
  // Construct the log file path using the outputPath from the Output class
  std::stringstream sslogFileName;
  sslogFileName << output.GetOutputPath() << "/idefix." << idfx::prank << ".log";
